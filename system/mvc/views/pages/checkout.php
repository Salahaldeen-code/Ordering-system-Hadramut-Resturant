...
<div id="checkout-box" class="container-fluid p-0">
	<div class="row m-0">
		<div class="p-2 bg2" style="height: 100vh;width: 320px;">
			<p class="ml-1 font-weight-bold cl" style="font-size: 130%">Bander Hadramout</p>
				...
				<table id="get-html-table" class="table bg2 cl">
					...
					<tbody>
						...
						foreach ($data["GetOrderCheckout"] as $value) {
							...
							<td>RM '.$value["price"]*$value["quanlity"].'</td>
							</tr>';
						}
						?>
...
<div id="print" class="mt-2 p-4 shadow-ok" style="width: 350px;height: 500px;display: none;">
	<p class="text-center">23-06-20230 Code Order #3</p>
	<p class="mb-0">PosSystem</p>
	<p class="mb-0">Phone: +601139282725</p>
	<p>Manage : Aziz</p>
	...
	<div>
		...
		foreach ($data["GetOrderCheckout"] as $value) {
			...
			<p class="float-right mb-0">RM '.$value['quanlity']*$value['price'].'</p>
			</p>
			<div style="clear: both;"></div>';
		}
		?>
	...
	<p id="discount-bill" class="float-right mb-0 mt-3">RM 5</p>
	<div style="clear: both;"></div>
...
<script type="text/javascript">
	...
	if(voucher == null){
		...
		$("#voucher").text('RM '+voucher);
		$("#voucher-bill").text('RM '+voucher);
	}
	$("#total").text('RM '+total);
	$("#total-bill").text('RM '+total);
	$("#return").val('RM '+total);
	...
	$("#received").keyup(function(){
		...
		$("#received-bill").text('RM '+received);
		$("#return").val('RM '+total-received*1);
		$("#return-bill").text('RM '+-1*(total-received*1));
